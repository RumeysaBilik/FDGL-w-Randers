#!/usr/bin/env python3
"""
run_mammoth_generated.py -- Randers Force-Directed Layout on the mammoth point cloud (mammoth.csv) +
a hand-crafted Randers field, using the EXACT SAME pipeline as
run_swiss_roll_generated.py
...
Usage
-----
    python run_mammoth_generated.py
    python run_mammoth_generated.py --n 1500 --epochs 500
...
def main():
    ...
    p.add_argument("--adjacency", choices=["threshold", "knn"], default="knn",
                    help="how "
                         ...
                         "'threshold' = symmetric by construction.")
    ...
    p.add_argument("--force-model", choices=["fr_gravity", "umap"], default="fr_gravity",
                    help="see run_swiss_roll_generated.py's --force-model help -- "
                         "'fr_gravity' (default) = Bannister et al.'s own "
    ...
    if args.init_only:
        ax.set_title(f"Randers Force-Directed Layout mammoth, LOCATED INIT ONLY (no training)  (n={n})", fontsize=11)
    else:
        ax.set_title(f"Randers Force-Directed Layout mammoth, located-drift init  "
    ...
    if args.snapshot_every is not None and not args.init_only:
        ...
        fig2.suptitle(f"Randers Force-Directed Layout mammoth, apply-step trajectory  (n={n}, "
