#!/usr/bin/env python3
"""
run_sphere_radial_generated.py -- Randers-UMAP on a synthetic sphere point cloud + a
RADIAL Randers field, using the EXACT SAME pipeline as run_swiss_roll_generated.py /
run_mammoth_generated.py / run_sphere_tangential_generated.py (fdgl_pipeline, imported
directly; make_sphere_points, imported from run_sphere_tangential_generated.py --
...
run_sphere_tangential_generated.py's field, and why that is the point of having both
...
reflects the underlying geometry, run_sphere_radial_generated.py's recovered drift
should correlate much more weakly with the true field than
run_sphere_tangential_generated.py's does, even though both fields have identical
...
def main():
    ...
    if args.init_only:
        ax.set_title(f"Randers-UMAP sphere (radial), LOCATED INIT ONLY (no training)  (n={n})", fontsize=11)
    else:
        ax.set_title(f"Randers-UMAP sphere (radial), located-drift init  "
