#!/usr/bin/env python3
"""
run_sphere_radial_generated.py -- Randers Force-Directed Layout on a synthetic sphere point cloud + a
RADIAL Randers field, using the EXACT SAME pipeline as run_swiss_roll_generated.py /
run_mammoth_generated.py / run_sphere_tangential_generated.py .
...
Usage
-----
    python run_sphere_radial_generated.py
    python run_sphere_radial_generated.py --n 2000 --epochs 500
"""
...
def make_sphere_radial_randers(n, seed=42, radius=10.0, alpha=0.5):
    ...
    omega : (n,3) Randers drift vectors, ||omega_i|| == alpha (constant --
            same magnitude convention as run_sphere_tangential_generated.py, so the
...
def main():
    ...
    # ---- convention as run_sphere_tangential_generated.py / run_mammoth_generated.py. --------------
    ...
    if args.init_only:
        ax.set_title(f"Randers Force-Directed Layout sphere (radial), LOCATED INIT ONLY (no training)  (n={n})", fontsize=11)
    else:
        ax.set_title(f"Randers Force-Directed Layout sphere (radial), located-drift init  "
    ...
    if args.snapshot_every is not None and not args.init_only:
        ...
        fig2.suptitle(f"Randers Force-Directed Layout sphere (radial), apply-step trajectory  (n={n}, "
